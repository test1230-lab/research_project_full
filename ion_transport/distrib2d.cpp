...
// vi_par = sqrt(vf_par^2 - 2*a*dz). with a < 0 (deceleration) and dz > 0 the
// radicand is always positive, so no domain guard is needed here -- and this
// runs inside omp parallel regions, where a throw would terminate.
